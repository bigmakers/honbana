...
■ みんなの図書館
本にコメントを公開して、みんなの感想と並べられる巨大な図書館。
気になる読み手をフォローすれば、その人の本棚の動きが「フォロー中」
フィードに流れてきます。もちろん全体タイムラインも。

【データの取り扱い】
書誌情報は openBD・国立国会図書館サーチ・Google Books の公開 API から
取得しています。あなたのメモや写真は端末内のみに保存されます。
「みんなの図書館」に投稿したコメントとニックネームのみ、iCloud
(CloudKit) を通じて全ユーザーに公開されます。
```
...
| 質問 | 回答 |
|---|---|
| ユーザーから収集する情報 | **ユーザーコンテンツ**（1.2.0 以降: 公開コメント・ニックネーム） |
| 収集データとユーザーの紐付け | あり（CloudKit のユーザーレコード ID 単位） |
| 第三者ライブラリで収集 | **なし** |
| トラッキング (App Tracking Transparency) | **なし** |

理由:
- ログインなし、アカウント作成なし（iCloud サインインを利用）
- 書誌取得時に ISBN を openBD/NDL/Google Books に送るがユーザー識別子は送らない
- メモ・画像はすべて端末内 SwiftData にのみ保存
- 1.2.0 で追加した「みんなの図書館」のコメントとニックネームのみ CloudKit パブリックDBに保存され全ユーザーに公開される（App プライバシーでは「ユーザーコンテンツ > その他のユーザーコンテンツ」を「アプリの機能」目的で申告）
